...
| File | Description |
|------|-------------|
| **StatusBarController.swift** | Menu bar UI. Manages NSStatusItem icon and dropdown menu: transcribe/translate buttons, copy & paste last transcription, copy & paste last translation, settings entry, about, quit. Dynamically updates icon and menu items based on app state. Defines `ApiMode` and `AutoSendMode` enums. Subscribes to LocaleManager for locale-aware menu refresh |
