...
| **Log.swift** | Logging utility. `Log.i/w/e/d()` outputs to both console and file `whisperutil.log`. Log language controlled by `EngineeringOptions.logLanguage` via `LocaleManager.logLocalized()` |
| **TextPostProcessor.swift** | Stateless text post-processing. Tag filtering (remove `[MUSIC]`/`[BLANK_AUDIO]` etc., controlled by `EngineeringOptions.enableTagFiltering`), Chinese script conversion (Traditional/Simplified based on `SettingsStore.whisperLanguage`), whitespace trimming. Used by RecordingController and TranslationPipeline |
