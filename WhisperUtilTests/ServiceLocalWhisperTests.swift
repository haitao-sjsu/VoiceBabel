// ServiceLocalWhisperTests.swift
// WhisperUtilTests
//
// ServiceLocalWhisper 依赖 WhisperKit + 626MB CoreML 模型。
...
final class LocalWhisperInitTests: XCTestCase {
    ...
    func testNotReady_beforeModelLoaded() {
        let service = ServiceLocalWhisper(language: "zh")
    ...
    func testLanguage_preserved() {
        let service = ServiceLocalWhisper(language: "en")
        XCTAssertEqual(service.language, "en")
    }

    func testEmptyLanguage_meansAutoDetect() {
        let service = ServiceLocalWhisper(language: "")
...
final class LocalWhisperErrorPathTests: XCTestCase {
    ...
    func testTranscribe_withoutModel_throws() async {
        ...
        do {
            ...
            XCTAssertTrue(error is ServiceLocalWhisper.LocalWhisperError)
        }
    }
...
final class LocalWhisperErrorTypeTests: XCTestCase {
    ...
    func testErrorDescriptions_nonEmpty() {
        let errors: [ServiceLocalWhisper.LocalWhisperError] = [
            .modelNotLoaded,
            .noAudioData,
    ...
    func testTranscriptionFailed_carriesReason() {
        let error = ServiceLocalWhisper.LocalWhisperError.transcriptionFailed("timeout")
