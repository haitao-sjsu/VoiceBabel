...
// WhisperUtilTests
//
// 【为什么 RecordingController 难以完整测试？】
//
// 想象你要测试"录音中按 ESC → 状态回到 idle"这个场景。
// 你需要先让 RecordingController 进入 recording 状态。
...
//
// 第三步：RecordingController 依赖 protocol
...
//       func stopRecording() -> RecordingResult? { return fakeResult }
//   }
//
...
// MARK: - RecordingController 初始化测试
//
// 【讲解】创建真实的 RecordingController 需要 AudioRecorder（AVAudioEngine），
...
final class AppStateTests: XCTestCase {
    ...
    func testAllStatesExist() {
        // 编译通过即验证了所有 case 都存在
        let states: [RecordingController.AppState] = [
